...
管理面板「高可用管理」页（`/admin/ha`）提供手动主/备切换，复用上面的交接协议，只是请求来源不同：

| 操作位置 | 实现 | 说明 |
|---|---|---|
| 在 primary 上"交出领导权" | 纯进程内标志（`manualHandoff`），**不写任何 TXT** | handoff 记录存在的唯一理由是副本没有别的通道通知主节点；请求直接敲在 owner 上时无需绕行 Cloudflare，单写入者纪律零扰动 |
| 在 replica 上"升本机为主" | 复用 `_ha-handoff` 挑战者协议，跳过 Preferred 与 60s 观察期检查 | 记录写入统一收口在 Agent tick 循环，写入者集合与自动回切完全一致 |

两种模式：

- **平滑交接**（默认）：与自动回切同一流程——等 audio 流结束（上限 30 分钟）→ 停写 → 对端追平 → 让位；
- **强制切换**：`Handoff` 记录新增 `force=1` 字段（旧版本解析时忽略未知键，滚动升级期自动退化为平滑），**只跳过等流阶段**；停写 → 追平 → 交接的零数据丢失流程与 90s 停写兜底不受影响。

其它要点：

- 全程异步：API（`POST /api/v1/admin/ha/switch`）只置位即返回，实际动作发生在之后的 tick，响应不会被进程退出截断；
- 该组路由不挂 `RequirePrimary` 写闸门（replica 侧"升主"只写 Cloudflare 不写本地库），也因此避开前端对 `NODE_READ_ONLY` 503 的自动重试重放；
- **自动回切抑制**：把 preferred 节点手动切走后，若不做处理，~60s 内 `maybeRequestFailback` 会把领导权自动要回来。因此 preferred 节点在进入停写（`beginDrain`）前把 `system_settings.haAutoFailback` 写成 `false`——写入发生在对端追平校验之前，保证一定随复制同步到新主；管理面板提供恢复开关。preferred 节点让位只可能出自手动切换（自动回切永远是把领导权交还给 preferred），所以这个钩子不影响自动流程；
- 手动切换绕过 120s 切换冷却（管理员显式操作），但完成时照常记录切换时刻，其后的**自动**决策仍受冷却抑制；
- 手动切换因对端异常中止时会记录原因（状态 API 的 `aborted` 阶段），供管理面板展示。
