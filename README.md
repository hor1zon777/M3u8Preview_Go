...
- **字幕**：默认走远程 GPU Worker（[m3u8-preview-worker](../m3u8-preview-worker) Tauri 桌面端），服务器只做任务池 + VTT 仓库；保留本地 whisper.cpp 兼容模式
...
- [FAQ / 故障排查](#faq--故障排查)
- [开发命令](#开发命令)
- [字幕功能（远程 GPU Worker 架构）](#字幕功能远程-gpu-worker-架构)
...
| `SUBTITLE_WORKER_STALE_MINUTES` | `10` | 远程 worker 心跳超时（1-120 分钟），超过后任务被回收回 PENDING |
